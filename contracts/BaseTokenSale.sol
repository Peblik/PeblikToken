...
contract BaseTokenSale is Pausable {
    ...
    */
    ...
    function calcCentsToWei(uint256 centsAmount) public view returns (uint256 value) {
        uint256 weiAmount = centsAmount.mul(1 ether).div(centsPerEth);
        return weiAmount;
    }

    function calcCentsToTokensValidate(address buyer, uint256 centsAmount) public view returns (bool value) {
        if (buyer == address(0x0)) {
            return false;
        }
        if (!validPurchase(buyer)) {
            return false;
        }
        if (msg.sender != paymentSource) {
            return false;
        }

        require(buyer != address(0x0));
        require(validPurchase(buyer));
        require(msg.sender == paymentSource); // transaction must come from pre-approved address


        // transaction must come from pre-approved address
        uint256 totalAmount = centsAmount;
        uint256 newBuyer = 0;
     
        if (totalPurchase[buyer] != 0) {
            totalAmount = totalAmount.add(centsAmount);
            newBuyer = 1;
        }
        
        if (centsAmount < minCents) {
            // single purchase must meet the minimum
            //PurchaseError("Below minimum purchase amount.", msg.sender);
            return false;
        } else if (totalAmount > maxCents) {
            // total of all purchases by a single buyer during the sale cannot exceed the max
            //PurchaseError("Above maximum purchase amount.", msg.sender);
            return false;
        }
        
        uint256 price = getDollarPrice(centsAmount,0,0, buyer);
        // Price should never be zero, but just in case.
        if (price == 0) {
            //PurchaseError("Price is zero.", msg.sender);
            return false;
        }

        uint256 tokens = centsAmount.mul(10 ** token.decimals()).div(price);
        if (tokens <= 0 ) {
            //PurchaseError("Zero Tokens created.", msg.sender);
            return false;
         }
    ...
    function getcapReached() public view returns (bool value) {
        return capReached;
    }
    ...
    function getTokensSold() public view returns (uint256 value) {
        return tokensSold;
    }

    function getTokenCap() public view returns (uint256 value) {
        return tokenCap;
    }

    function getStartTime() public view returns (uint256 value) {
        return startTime;
    }

    function getEndTime() public view returns (uint256 value) {
        return endTime;
    }

    function getConversionRate() public view returns (uint256 value) {
        return centsPerEth;
    }
}